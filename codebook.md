...
There were initially 561 measurements in the raw data. From this full dataset, we filtered it down to only the measurements of mean and standard deviation (name includes either exactly `mean()` or `std()`). This left us with 66 measurements (shown below).

We then grouped the data by `Subject` & `Activity`

`Subject` - An integer between `[1-30]` identifying the person who was measured

`Activity` - A factor with 6 values `[WALKING, WALKING_UPSTAIRS, WALKING_DOWNSTAIRS, SITTING, STANDING, LAYING]` indicating the activity of the subject
